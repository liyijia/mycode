$(function () {
    ...
    $('#link').delegate('input[type=checkbox]', 'click', function () {
        if ($(this).prop('checked')) {
            $('#content').insertContent('{' + $(this).next().html() + '}');
        } else {
            $('#content').val($('#content').val().replace('{' + $(this).next().html() + '}', ''));
        }
    });

    $('#chkItem').delegate('input[type=checkbox]', 'click', function() {
        if ($(this).prop('checked')) {
            $('#content').insertContent($(this).val());
        } else {
    ...
    $('#btnSelect').click(function () {
        if (page.contact.is(':visible')) {
            ...
            if (flag == undefined) {
                ...
                $.post('/Message/StudentGroup', {}, function (data) {
                    var i = 0;
                    $.each(data, function (index, target) {
                        var tag = '<input id=\'chkGroup' + index + '\' type="checkbox" value=\'' + target.Id + '\' style="float: left;" />' +
                                  '<label for=\'chkGroup' + index + '\' style="float: left; margin:0 5px 0 0;">' + target.Name + '</label>';
                        ++i;
                        if (i == 5) {
                            tag += '<div class=\'clear\'></div><br />';
                            i = 0;
                        }
                        
                        $studentGroup.append(tag);
                    });
                }, 'json');

                var $student = $('#student');
                $.post('/Message/GetStudent', {}, function (data) {

                    var j = 0;
                    $.each(data, function (index, target) {
                        var gid = '';
                        $.each(target.Group, function (i, n) {
                            gid += n.Id + ' ,';
                        });
                        gid = gid.substring(0, gid.length - 1);
                        var tag = '<input id=\'chkStudent' + index + '\' itemid=\'' + gid + '\' type="checkbox" value=\'' + target.Id + '\' style="float: left;" />' +
                                  '<label for=\'chkStudent' + index + '\' style="float: left; margin:0 5px 0 0;">' + target.Name + '</label>';
    ...
    $('#btnSend').click(function () {
        ...
        $.ajax({
            type: 'post',
            url: '/Message/SendExam',
            data: $("form").serialize(),
            datatype: 'json',
            success: function (data) {
                $.messager.alert('提示信息', '信息发送成功!一共成功发送 ' + data.Count + ' 条信息');
            },
